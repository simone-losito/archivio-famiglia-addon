...
$ocrTags = '';
$ocrSuggestedTitle = '';
$ocrSuggestedDate = null;

if (in_array($ext, ['jpg', 'jpeg', 'png', 'webp'], true)) {
    $ocrText = runOcrOnImage($dest);
    $ocrSuggestion = buildOcrSoftSuggestion($ocrText);
    $ocrTags = $ocrSuggestion['tags'];
    $ocrSuggestedTitle = $ocrSuggestion['title'];
    $ocrSuggestedDate = $ocrSuggestion['date'];

    if ($ocrTags !== '') {
        $tags = trim($tags . ($tags !== '' ? ', ' : '') . $ocrTags);
    }

    if ($dataDocumento === null && $ocrSuggestedDate !== null) {
        $note = trim($note . ($note !== '' ? "\n" : '') . 'OCR data suggerita: ' . $ocrSuggestedDate);
    }

    if ($ocrSuggestedTitle !== '' && cleanText((string)($_POST['titolo'] ?? ''), 180) === '') {
        $note = trim($note . ($note !== '' ? "\n" : '') . 'OCR titolo suggerito: ' . $ocrSuggestedTitle);
    }
}
